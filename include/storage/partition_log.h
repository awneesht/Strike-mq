...
public:
    ...
    [[nodiscard]] int64_t high_watermark() const { return next_offset(); }
...
private:
    ...
    static uint32_t serialize_batch(const RecordBatch& batch, uint8_t* buf, uint32_t cap) {
        ...
        // Use strike::bswap* from utils/endian.h (no protocol:: dependency)
        auto w64 = [&](int64_t v) {
            uint64_t be = strike::bswap64(static_cast<uint64_t>(v));
            std::memcpy(buf + pos, &be, 8); pos += 8;
        };
        auto w32 = [&](int32_t v) {
            uint32_t be = strike::bswap32(static_cast<uint32_t>(v));
            std::memcpy(buf + pos, &be, 4); pos += 4;
        };
        auto w16 = [&](int16_t v) {
            uint16_t be = strike::bswap16(static_cast<uint16_t>(v));
        ...
        uint32_t bl = pos - 12;
        uint32_t bel = strike::bswap32(bl);
