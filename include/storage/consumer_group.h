...
public:
    std::vector<GroupInfo> list_groups() {
        std::lock_guard<std::mutex> lock(mu_);
        std::vector<GroupInfo> result;
        for (const auto& [id, g] : groups_) {
            result.push_back({id, state_name(g.state), g.generation_id, g.members.size()});
        }
        return result;
    }

    std::optional<GroupDetail> get_group(const std::string& group_id) {
        std::lock_guard<std::mutex> lock(mu_);
        ...
        d.state = state_name(g.state);
        d.generation_id = g.generation_id;
        d.protocol_type = g.protocol_type;
        d.protocol_name = g.protocol_name;
        d.leader_id = g.leader_id;
        for (const auto& [_, m] : g.members) d.members.push_back(m);
        d.committed_offsets = g.committed_offsets;
        return d;
    }
